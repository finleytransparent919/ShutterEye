...
On the HW side of things, lets just say that I don't understand how manufacture can suggest almost impossible amount of bypass capacitors as reference design, like did you(The one who draw such sch) even think about layout? There is no Fway that I can put 10uF + 0.1uF on EVEY-SINGLE-PWR-PIN, so in the sch you will see I shrink down to 1uF each. Another issue rise with such a capacitor array is the in rush current when powered on, specifically Raspberry Pi 5 has a very sensitive overcurrent detection, which can be a good thing for short circuit detection but in this case in my beta version of the board it trips that OC detection with that array of capacitors all rails turned on at once by the enable switch. So the solution I have to implment is soft start for all the power rails other then VDDIO, mostly VDDA and VDDP has the largest soft start time delay. Now I haven't even finish yapping about the capacitors yet because there is still charge pump capacitors as global shutter image sensor tends to require more power and more power rails, one of the issue(?) is that a bunch of rails like VRM/VRSFLO... have like two pins SEPERATED on differnt side of the chip, where I need to bridge them together. Because of the pin density and layout reasons it is not a large power plane but just a few narrow traces connect them together, not ideal but it seems to be working just fine.

As for the OSC frequency I have to go with 27MHz because the datasheet I have didn't even bother documenting the PLL configuration, and the only thing I have is initializing register list that said it is for 27MHz. Finally I have the L0~L3 + G0,G1 LED strobe and GPIO breakout but I have no idea how to get it to work (I tried).

On the driver side of things it is setup quite easily, what I really like is the sensor built-in analog gain can be quite high, which is handy for global shutter sensor where you need very quick exposure speed but you've maxed out the lighting you have, and having the capability to just crank up the gain is nice, though yes the noise too.

Other then the lack of some critical details, most of the (normal) sensor functions should works and the driver side of things were quite easy to code also, haven't tried the HDR features yet but there seems to be some trick up its sleeves with outputing different gain readout as two lines, so I guess you can kinda treat is as quad bayer while in the RAW data collection you can do your own HDR while the preview maybe still useable.
